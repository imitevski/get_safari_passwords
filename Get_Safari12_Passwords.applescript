	...
	tell application "System Events" to tell application process "Safari"
		...
		tell theTable
			...
			repeat while row_index ² nRows
				...
				tell application "System Events" to tell application process "Safari"
					local theSite, theName, theUser, thePass, urlList, rowValues, theSheet
					set {theTitle, theSite, theUser, thePass} to {"Untitled", "", "", ""}
					set urlList to {}
					...
					if theSheet is not 0 then
						...
						set theNotes to Join(character id 10, urlList) of me
						local tmpList
						set tmpList to {theTitle, theSite, theUser, thePass, theNotes}
...
on write_csv_file(Entries, fpath)
	local rowdata
	set beginning of Entries to {"Title", "Login URL", "Login Username", "Login Password", "Notes"}
